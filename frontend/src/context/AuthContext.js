...
export function AuthProvider({children}){
    ...
    useEffect(()=>{
        let savedAuth = localStorage.getItem('authenticated') ? JSON.parse(localStorage.getItem('authenticated')) : {loggedIn: false, tokens:null}
        setAuth(savedAuth)
    },[])
    ...
    return (
        <AuthContext.Provider value={auth}>
            <AuthUpdateContext.Provider value={updateAuth}>
                <AuthGetUser.Provider value={user}>
                    {children}
                </AuthGetUser.Provider>
